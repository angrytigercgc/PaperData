...
--*/
{
	...
	AddLog("==> Passthru Protocol Initialize\n");
...
--*/
{
	...
	AddLog("==> Passthru PtUnbindAdapter\n");
	...
	else do
	{
		...
		if(Packet != NULL)
		{
			 ...
			 if(Status == NDIS_STATUS_SUCCESS)
			 {
				...
				sprintf(str,"=====> RECV UNKNOWN !\n");	AddLog(str);
/*
//		sprintf(str,"PassPacket: ...\n");	AddLog(str);
		sprintf(str,"Count = %d, Flag = %X, NdisPacketFlags = %X\nPhysicalCount = %d, TotalLength = %d, ValidCounts = %d\n NdisPacketOobOffset = %d\n",
			Packet->Private.Count,
			Packet->Private.Flags,
			Packet->Private.NdisPacketFlags,
...
*/
		...
		pBuf = (unsigned char*)HeaderBuffer;
		NdisMoveMemory(CopyBlock, pBuf, HeaderBufferSize);
		pBuf = (unsigned char*)LookAheadBuffer;
		NdisMoveMemory(&CopyBlock[HeaderBufferSize], pBuf, LookAheadBufferSize);

//		CopyBlock[BLOCK_SIZE];
/*
		pBuf = (unsigned char*)HeaderBuffer;
		sprintf(str,"HEAD: %d : %X:%X:%X, %X:%X:%X",
			HeaderBufferSize,pBuf[3],pBuf[4],pBuf[5],pBuf[9],pBuf[10],pBuf[11]);AddLog(str);
		if (pBuf[0] != 0xFF)
		{
//			pBuf[3]=0x78;
//			pBuf[4]=0x03;
//			pBuf[5]=0x08;
		...
		pBuf = (unsigned char*)LookAheadBuffer;
		sprintf(str,"LOOK: %d : %X:%X:%X, %X:%X:%X\nTotal Len=%d\n",
			LookAheadBufferSize,pBuf[0],pBuf[1],pBuf[2],pBuf[3],pBuf[4],pBuf[5],PacketSize);AddLog(str);
*/
		sprintf(str,"old Len=%d\n",PacketSize);
...
--*/
{
	...
	AddLog("PtPnPNetEventReconfigure\n");
	if(pAdapt == NULL)
	{
