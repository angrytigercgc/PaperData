...
#define FILTER_DEVICE_CODE        0x8308
#define MAX_PASS_PACKET			10

#define IOCTL_FILTER_LOG_WRITE		CTL_CODE(FILTER_DEVICE_CODE, 0 , METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_FILTER_SET_TEST		CTL_CODE(FILTER_DEVICE_CODE, 1 , METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_FILTER_GET_TEST		CTL_CODE(FILTER_DEVICE_CODE, 2 , METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct _PASS_PACKET
{
	PNDIS_PACKET	Packet;
	PNDIS_BUFFER	Buffer;
	PUCHAR			Block;
	UINT			BlockLen;
//	bool			Using;
} PASS_PACKET, *PPASS_PACKET;

//advance declaration
typedef struct _ADAPT ADAPT, *PADAPT;

#define LOG_FILE_NAME L"\\DosDevices\\C:\\PASSLOG.TXT"
#define NT_DEVICE_NAME L"\\Device\\NFilterI"
#define DOS_DEVICE_NAME L"\\DosDevices\\NFilterI"
#define IS_UNUSED(_p) ((PPASS_PACKET)_p)->Packet!=NULL

extern void InitLog();
extern void AddLog(char *str);
extern void WriteLog(char *str);

extern PPASS_PACKET FindPacketFromOrgPacket(PNDIS_PACKET pPacket);
extern PPASS_PACKET FindUnusedPacket();
...
extern PASS_PACKET	gPackets[MAX_PASS_PACKET];
